...
const toneClass: Record<ServiceCardTone, string> = {
  silver: 'from-bone-50/14 via-ink-850/92 to-ink-1000 group-hover:border-bone-50/38',
  white: 'from-bone-50/10 via-ink-850/90 to-ink-1000 group-hover:border-bone-50/32',
  steel: 'from-steel-500/16 via-ink-850/92 to-ink-1000 group-hover:border-steel-500/45',
}
...
export function ServicesSection() {
  ...
  return (
    ...
    >
      <motion.div
        className="mt-12 grid auto-rows-[minmax(16rem,auto)] grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-12"
        initial="hidden"
          ...
          return (
            ...
            >
              {image ? (
                <img
                  alt=""
                  className="absolute inset-0 h-full w-full object-cover opacity-48 grayscale contrast-110 saturate-75 transition duration-500 group-hover:scale-[1.04] group-hover:opacity-62 group-hover:saturate-90"
                  src={image}
                />
              ) : null}
              <div
                className={cn(
                  'absolute inset-0 bg-gradient-to-br opacity-92 transition duration-500 group-hover:scale-[1.03]',
                  visual && toneClass[visual.tone],
                )}
              />
              <div className="absolute inset-0 bg-gradient-to-t from-ink-1000/82 via-ink-950/36 to-ink-1000/28" />
              ...
              <div className="relative z-10 flex h-full min-h-full flex-col">
                ...
                {image ? (
                  <button
                    aria-label={`Ver imagen de ${service.name}`}
                    ...
                    onClick={() =>
                      setSelectedImage({
                        category: visual?.label ?? 'Servicio',
                        image,
                        title: service.name,
                      })
                    }
